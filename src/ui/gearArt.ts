...
const rect = (rx: number, ry: number, w: number, h: number, fill: string, extra = '') =>
  `<rect x="${f(rx)}" y="${f(ry)}" width="${f(w)}" height="${f(h)}" fill="${fill}"${x(extra)}/>`;
const rr = (rx: number, ry: number, w: number, h: number, r: number, fill: string, extra = '') =>
...
const path = (d: string, fill: string, extra = '') => `<path d="${d}" fill="${fill}"${x(extra)}/>`;
const line = (d: string, color: string, w = 1.5, extra = '') =>
...
const stroke = (color: string, w = 1) => `stroke="${color}" stroke-width="${w}"`;
const group = (inner: string, extra: string) => `<g ${extra}>${inner}</g>`;
const shadow = (rx = 14, cy = 44.5) => ell(24, cy, rx, 2, '#000', op(0.35));
const glow = (cx: number, cy: number, r: number, c: string, o = 0.22) => circ(cx, cy, r, c, op(o));
const spark = (sx: number, sy: number, s: number, fill: string) =>
  path(
...
function grid(x0: number, y0: number, cols: number, rows: number, w: number, h: number, gap: number, fill: string, extra = ''): string {
  let s = '';
...
function grid2(x0: number, y0: number, cols: number, rows: number, w: number, h: number, gx: number, gy: number, fill: string, extra = ''): string {
  let s = '';
  for (let r = 0; r < rows; r++) for (let k = 0; k < cols; k++) s += rr(x0 + k * (w + gx), y0 + r * (h + gy), w, h, 0.6, fill, extra);
  return s;
}
const stars = (pts: [number, number][], fill = K.white) => pts.map(([sx, sy], i) => circ(sx, sy, i % 3 === 0 ? 0.9 : 0.55, fill)).join('');
const rings = (cx: number, cy: number, radii: number[], c: string) =>
