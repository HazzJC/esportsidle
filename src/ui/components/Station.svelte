...
<g transform="translate({x} {y}) scale({scale})" class="station" class:out={status === 'out'}>
  <defs>
    ...
    <linearGradient id="{uid}-screen" x1="0" y1="0" x2="1" y2="1">
      ...
      <stop offset="1" stop-color="#0b0e1f" />
    </linearGradient>
  </defs>
  ...
  {:else if gear.chair <= 2}
    <rect x="-24" y="-96" width="48" height="56" rx="9" fill="#2d3348" />
    <rect x="-3" y="-40" width="6" height="40" fill="#1a1d29" />
    <path d="M-20 2 L20 2" stroke="#1a1d29" stroke-width="5" stroke-linecap="round" />
  {:else if gear.chair <= 5}
    ...
    <rect x="-3" y="-40" width="6" height="40" fill="#15171f" />
    <path d="M-22 2 L22 2" stroke="#15171f" stroke-width="5" stroke-linecap="round" />
  {:else}
    <path d="M-31 -40 L-34 -126 Q0 -142 34 -126 L31 -40 Z" fill={shade(primary, -0.2)} stroke="#ffc83d" stroke-width="2.5" />
    <path d="M-12 -128 L0 -140 L12 -128" fill="none" stroke="#ffc83d" stroke-width="2.5" />
    {#if gear.chair >= 10}
      <ellipse cx="0" cy="-2" rx="30" ry="5" fill="#22e4ff" opacity="0.55" filter="url(#{uid}-glow)" />
    {:else}
      <rect x="-4" y="-40" width="8" height="40" fill="#15171f" />
    {/if}
  {/if}
  ...
  {:else if gear.desk <= 8}
    <path d="M-74 -64 H74 L70 -52 H-70 Z" fill="#1b1f33" />
    <path d="M-72 -52 H72" stroke={secondary} stroke-width="2" filter="url(#{uid}-glow)" />
    <path d="M-62 -52 L-70 0 M62 -52 L70 0" stroke="#11131f" stroke-width="7" />
  {:else}
    <path d="M-78 -64 H78 L72 -50 H-72 Z" fill="rgba(34,228,255,0.18)" stroke="#22e4ff" stroke-width="2" filter="url(#{uid}-glow)" />
    <path d="M-60 -50 L-40 0 H40 L60 -50" fill="rgba(34,228,255,0.06)" stroke="rgba(34,228,255,0.35)" stroke-width="1.5" />
  {/if}

  <!-- Keyboard & mouse -->
  <rect x="-20" y={gear.desk === 0 ? -61 : -67} width="40" height="5" rx="1.5" fill={gear.keyboard >= 5 ? '#1b1f33' : '#d9d6cf'} />
  {#if gear.keyboard >= 5}
  ...
  {#snippet monitor(cx: number)}
    ...
    {:else if gear.monitor <= 10}
      ...
      <rect x={cx - 2} y={baseY - 10} width="4" height="10" fill="#15171f" />
      <rect x={cx - 10} y={baseY - 2} width="20" height="3" rx="1" fill="#15171f" />
      <rect x={cx - w / 2} y={baseY - 42} width={w} height="32" rx="2" fill="#0f1119" />
  ...
  {:else if gear.pc <= 5}
    <rect x="76" y="-50" width="24" height="50" rx="3" fill="#16181f" />
  ...
  {:else if gear.pc <= 8}
    <rect x="74" y="-62" width="30" height="62" rx="3" fill="rgba(20,24,40,0.9)" stroke={secondary} stroke-width="1.5" />
    <rect x="78" y="-58" width="22" height="54" rx="2" fill={secondary} opacity="0.25" filter="url(#{uid}-glow)" />
  {:else if gear.pc <= 11}
    <rect x="72" y="-86" width="34" height="86" rx="2" fill="#0d0f18" stroke="#2c3150" stroke-width="1.5" />
    {#each [0, 1, 2, 3, 4, 5] as i (i)}
      <rect x="76" y={-80 + i * 13} width="26" height="8" rx="1" fill="#1a1e30" />
      <circle cx="98" cy={-76 + i * 13} r="1.6" fill={i % 2 ? '#3dff9a' : '#22e4ff'} class="blink" style="animation-delay:{i * 0.3}s" />
    {/each}
  {:else}
    <circle cx="90" cy="-40" r="16" fill="rgba(255,43,214,0.35)" stroke="#ff2bd6" stroke-width="2" filter="url(#{uid}-glow)" class="orb" />
    <circle cx="90" cy="-40" r="6" fill="#fff" opacity="0.8" />
  {/if}

  <!-- Status badge -->
  <g transform="translate(30 -120)">
    <circle r="11" fill={badge.fill} stroke="#0b0e1f" stroke-width="2" />
