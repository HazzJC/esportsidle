...
export const NEWS: NewsItem[] = [
  ...
  { text: 'Invitational trophy so heavy the winning team needs a physio to lift it.', when: played() },
  ...
  { text: '{rival} claim their last grudge match loss was "a scheduling issue".', when: (s) => (s.rival?.losses ?? 0) >= 1 },
  { text: '{rival} coach spotted taking notes outside the {org} house. Notes read "they have a cat".', when: (s) => s.rival !== null },
  { text: 'Grudge match week: {rival} and {org} both claim to be the underdog. Bookmakers give up.', when: (s) => s.rival !== null },
  ...
  // Absurd Gaming Tropes & Cookie Clicker style satire
  { text: 'Nutritionist horrified as {org} replaces all meals with tactical beef jerky and caffeinated jelly beans.' },
  { text: 'Match paused for 20 minutes to scrape extreme nacho cheese dust out of {player}’s mechanical switches.', when: played() },
  { text: 'Study finds 94% of tactical timeouts are actually just players unwrapping emergency snack bars.' },
  { text: '{org} pantry restocked with emergency gummy worms. Squad morale hits an all-time high.' },
  { text: 'Player’s RGB mousepad fails firmware update; now bricked and glowing a sad, mournful beige.' },
  { text: '{org}’s ergonomic gaming chair downloads a 4GB patch mid-round, tilts 45 degrees, and locks up.' },
  { text: 'Peripheral company unveils RGB water bottle. Hydration now syncs with your killstreak.' },
  { text: 'Smart mouse requires cloud login to right-click. Server outage leaves entire league paralyzed.' },
  { text: '{org} announces controversial new title sponsor: Gamer Bathwater Plus. Manager claims it improves APM.' },
  { text: 'Energy drink brand releases "Pure Streamer Essence" flavour. Reviewers describe it as "tasting like 3 AM".' },
  { text: 'Scandal erupts after sponsor’s miracle gaming serum turns out to be tap water and green food dye.' },
  { text: 'Streamer bathwater bathtub installed in {org} house. Physio refuses to enter room.', when: house },
  { text: 'Doctors express concern as {player} consumes 8th energy drink and begins dodging hitscan bullets before they are fired.', when: played() },
  { text: '{player} claims they can now hear electromagnetic waves after drinking experimental pre-workout.', when: played() },
  { text: 'Emergency medical team called after {player} attempts to brew espresso using energy drink instead of water.', when: played() },
  { text: 'Caffeine detector in {org} gaming house alarms non-stop. Nobody knows which player triggered it.', when: house },
  { text: 'Boot camp evacuated after heated three-hour debate over whose turn it was to wash the team shaker bottle.', when: has('bootcamp') },
  { text: '{org} coach threatens to confiscate all phone chargers until communication improves in scrims.', when: staff('coach') },
  { text: 'Boot camp drama reaches boiling point when someone drinks the marked almond milk from the mini-fridge.', when: has('bootcamp') },
  { text: 'Team bonding retreat descends into full-scale tactical warfare during a game of Uno.' },
  { text: 'Coach locks {org} squad in room until everyone agrees on what "he is one shot" actually means.', when: staff('coach') },

  // Game-Specific Headlines
  { text: 'Smash Siblings main brings a 40kg CRT television on a commercial flight as carry-on luggage.', when: hasTeam('smash') },
  { text: 'Smash Siblings community in shambles after new tier list places everyone’s main in F-tier.', when: hasTeam('smash') },
  { text: '{org} Smash player breaks controller celebrating a tense victory against a Level 3 CPU.', when: hasTeam('smash') },
  { text: 'Rocket Soccar squad spends four hours in video review analyzing why all three players rotated into the same corner.', when: hasTeam('rocket') },
  { text: 'Rocket Soccar aerial replay shows car spinning 14 times before completely missing an open net.', when: hasTeam('rocket') },
  { text: '{player} insists that catastrophic own goal was "a 4D chess passing play".', when: hasTeam('rocket') },
  { text: 'Counter-Stroke IGL calls an eco save round; AWPer immediately buys an AWP and zero armor.', when: hasTeam('counter') },
  { text: 'Counter-Stroke player throws a smoke grenade, bounces it off a door frame, and blinds entire team.', when: hasTeam('counter') },
  { text: 'Debate rages in Counter-Stroke: is inspecting your knife 4,000 times per round technically gameplay?', when: hasTeam('counter') },
  { text: 'League of Lanes mid laner dies once at 2 minutes, types "gg open mid", and begins studying for the bar exam.', when: hasTeam('lanes') },
  { text: 'League of Lanes jungler spotted farming raptors while base burns to the ground.', when: hasTeam('lanes') },
  { text: '{org} bot lane files formal restraining order against enemy four-man dive.', when: hasTeam('lanes') },
  { text: 'Apex Legumes squad blames 20th place finish on hot-dropping onto a single bin containing only a syringe.', when: hasTeam('apex') },
  { text: 'Apex Legumes player spends 15 minutes looting every gold attachment only to get beamed from 300 meters.', when: hasTeam('apex') },
  { text: '{org} third-partied by a squad that was already being third-partied by two other squads.', when: hasTeam('apex') },
  { text: 'Valorunt duelist flashes their entire team, whiffs every shot, then loudly blames 128-tick servers.', when: hasTeam('valorunt') },
  { text: 'Valorunt lineup nerd reveals 47-step lineup to land a molly directly into their own spawn.', when: hasTeam('valorunt') },
  { text: '{player} installs fourth anti-cheat system; PC now requires top-secret government clearance to boot.', when: hasTeam('valorunt') },
  { text: 'Fortnight builder constructs a 6-story hotel with Wi-Fi and plumbing upon hearing a single distant gunshot.', when: hasTeam('fortnight') },
  { text: '{org} player loses Fortnight duel because opponent did a copyright-infringing dance at them.', when: hasTeam('fortnight') },
  { text: 'Fortnight storm circle closes on a mountain; 12 players fall to their demise trying to edit stairs.', when: hasTeam('fortnight') },
  { text: 'StarCrafty commander logs 620 APM; medical review confirms 610 clicks were spamming worker waypoints.', when: hasTeam('starcrafty') },
  { text: 'StarCrafty match ends in 90 seconds after catastrophic worker rush catches everyone asleep.', when: hasTeam('starcrafty') },
  { text: '{org} commander complains opponent "built fighting units too early in a real-time strategy game".', when: hasTeam('starcrafty') },
  { text: 'Overclock payload remains stationary for 8 minutes while both teams brawl in an irrelevant side room.', when: hasTeam('overclock') },
  { text: 'Overclock support screams for help; team tanks continue chasing a single butterfly across the map.', when: hasTeam('overclock') },
  { text: '{org} Overclock roster switches to 6 DPS heroes. Team coach visibly ages 15 years.', when: hasTeam('overclock') },
  { text: 'Hearthstoned championship decided when random card effect summons a dinosaur that explodes both players.', when: hasTeam('hearthstoned') },
  { text: 'Hearthstoned grandmaster claims top-decking the exact lethal card on turn 10 was "pure strategic foresight".', when: hasTeam('hearthstoned') },
  { text: '{org} player ropes every turn to calculate odds that were already 100% loss.', when: hasTeam('hearthstoned') },
  { text: 'Quantum Pong paddle observed simultaneously in two positions; tournament judges consult quantum physicists.', when: hasTeam('pong') },
  { text: 'Quantum Pong match declared a tie after the ball tunnels straight through both paddles into another dimension.', when: hasTeam('pong') },
  { text: '{org} player masters the non-Euclidean curveball; opponents complain reality has an unfair bounce.', when: hasTeam('pong') },
  { text: 'Galactic Siege VR player punches drywall during heated dogfight; drywall files formal arbitration claim.', when: hasTeam('galactic') },
  { text: 'VR headset sweat barrier fails at match point; pilot navigates cosmic battle entirely by scent.', when: hasTeam('galactic') },
  { text: '{org} galactic vanguard tangles headset cord 14 times, accidentally invents new knot theory.', when: hasTeam('galactic') },

  // Dynamic Stat Highlights
  { text: '{org} has now fought through {matches} matches. Analysts predict only 10,000 more until basic competency.', when: (s) => s.stats.matchesWon + s.stats.matchesLost >= 50 },
  { text: 'With {wins} victories on the board, {org} fans insist they never doubted the team. Chat logs disagree.', when: (s) => s.stats.matchesWon >= 20 },
  { text: 'The trophy shelf holds {trophies} awards, yet {org} players still drink out of chipped promo mugs.', when: (s) => s.trophyCase.length >= 3 },
  { text: 'Team physio reports {injuries} treated ailments, ranging from carpal tunnel to devastating emotional defeats.', when: (s) => s.stats.injuries >= 5 },
  { text: 'Sociologists marvel at {org}’s {fans} fans, investigating whether collective delusion can power the electric grid.', when: (s) => s.fans >= 5000 },
  { text: '{matches} matches in, {org}’s keyboards have collectively travelled 300 miles in finger keystrokes.', when: (s) => s.stats.matchesWon + s.stats.matchesLost >= 100 },
  { text: 'Boasting {wins} wins, {org} releases commemorative jersey. Jersey is already sold out to scalper bots.', when: (s) => s.stats.matchesWon >= 200 },
  { text: 'With {trophies} trophies on display, the {org} trophy room has officially been upgraded to a walk-in vault.', when: (s) => s.trophyCase.length >= 15 },
  { text: 'Medical log notes {injuries} ailments treated. Physio recommends "looking at something green occasionally".', when: (s) => s.stats.injuries >= 15 },
];
