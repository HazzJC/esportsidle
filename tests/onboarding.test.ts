...
import { FIRST_DRAFT, signDraftPick } from '../src/engine/draft';
...
import { QUEST_SLOTS, claimQuest, fillQuests, questProgress, updateQuests } from '../src/engine/quests';
...
describe('a new org', () => {
  ...
  it('makes the cheapest prospect a quick click and the dearest a real grind', () => {
    const s = createNewGame(0, 1);
    const click = computeRates(s).click;
    const [cheap, , dear] = s.draft!;
    // At five clicks a second: the rookie in well under a minute, the pro in a few minutes.
    expect(cheap.price / click / 5).toBeLessThan(30);
    expect(dear.price / click / 5).toBeGreaterThan(120);
  });
...
describe('quests', () => {
  ...
  it('pays the chosen reward and brings in the next quest', () => {
    const s = foundedGame(0, 1);
    fillQuests(s);
  ...
  it('can pay a timed buff instead of cash', () => {
    const s = foundedGame(0, 1);
    fillQuests(s);
    s.ops.grinder.owned = 10;
    expect(claimQuest(s, 'grinders_10', 1, ctx(s), new Rng(s))).toBe(true);
    expect(s.buffs.some((b) => b.id === 'quest-click')).toBe(true);
  });

  it('every quest offers two different rewards', () => {
    expect(new Set(QUESTS.map((q) => q.id)).size).toBe(QUESTS.length);
    for (const q of QUESTS) {
      expect(q.rewards).toHaveLength(2);
      expect(q.rewards[0]).not.toEqual(q.rewards[1]);
    }
  });
